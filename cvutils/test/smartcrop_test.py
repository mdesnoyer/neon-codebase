...
class TestSmartCrop(unittest.TestCase):
    ...
    def test_face_crop(self):
        ...
        smart_crop = smartcrop.SmartCrop.get_cropper()
        # Crop horizontally
        cropped_im = smart_crop.crop(face_im, 300, 280)
        faces = smart_crop.face_cascade.detectMultiScale(cropped_im,
                                                         **smart_crop.haarParams)
        self.assertEqual(len(faces), 1)
        # Crop vertically
        cropped_im = smart_crop.crop(face_im,300, 130)
        faces = smart_crop.face_cascade.detectMultiScale(cropped_im, \
                                                         **smart_crop.haarParams)
        self.assertEqual(len(faces), 1)
    ...
    def test_saliency_crop(self):
        ...
        smart_crop = smartcrop.SmartCrop.get_cropper()
        cropped_im = smart_crop.crop(saliency_im, 300, 300)
