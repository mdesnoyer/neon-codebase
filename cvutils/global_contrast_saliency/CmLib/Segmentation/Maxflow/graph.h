	  ...
	  error_function(err_function)
{
	...
	char err_msg[] = "Not enough memory!";
	if (!nodes || !arcs) { if (error_function) (*error_function)(err_msg); exit(1); }
	...
	void Graph<captype,tcaptype,flowtype>::reallocate_nodes(int num)
{
	...
	char err_msg[] = "Not enough memory!";
	if (!nodes) { if (error_function) (*error_function)(err_msg); exit(1); }
	...
	void Graph<captype,tcaptype,flowtype>::reallocate_arcs()
{
	...
	char err_msg[] = "Not enough memory!";
	if (!arcs) { if (error_function) (*error_function)(err_msg); exit(1); }
	...
	flowtype Graph<captype,tcaptype,flowtype>::maxflow(bool reuse_trees, Block<node_id>* _changed_list)
{
	...
	char err_msg_1[] = "reuse_trees cannot be used in the first call to maxflow()!";
	if (maxflow_iteration == 0 && reuse_trees) { if (error_function) (*error_function)(err_msg_1); exit(1); }
	char err_msg_2[] = "changed_list cannot be used without reuse_trees!";
	if (changed_list && !reuse_trees) { if (error_function) (*error_function)(err_msg_2); exit(1); }
