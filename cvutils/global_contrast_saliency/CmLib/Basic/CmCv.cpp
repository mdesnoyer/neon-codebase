...
void CmCv::FFTShift(Mat& img)
{
	...
	Mat im1(img, Rect(0, 0, w, h));
	Mat im3(img, Rect(cx2, cy2, w, h));
	Swap(im1, im3); // swap 1, 3
	Mat im2(img, Rect(cx2, 0, w, h));
	Mat im4(img, Rect(0, cy2, w, h));
	Swap(im2, im4);  // swap 2, 4
}
...
Mat CmCv::GetBorderRegC(CMat &img3u, Mat &idx1i, vecI &idxCount)
{
	...
	Mat_<int> idx1iT;
	vecB vec_b;
	int regNum = CmCv::GetRegions(edg, idx1iT, idxCount, vec_b, true);
...
void CmCv::rubustifyBorderMask(Mat& mask1u)
{
	...
	vecI vec_i;
	vecB vec_b;
	int regNum = CmCv::GetRegions(mask1u, regIdx1i, vec_i, vec_b, true);
...
Mat CmCv::getGrabMask(CMat &img3u, Rect rect)
{
	...
	Mat masklu_roi(mask1u(Rect(rect.x+1, rect.y+1, rect.width-2, rect.height-2)));
	rubustifyBorderMask(masklu_roi);
	return mask1u;
}
