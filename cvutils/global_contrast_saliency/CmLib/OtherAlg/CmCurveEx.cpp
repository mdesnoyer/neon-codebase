...
void CmCurveEx::MergeResults(CMat der1f[3], CMat ornt1f[3], Mat &_der, Mat &_ornt)
{
	...
	for (int r = 0; r < sz.height; r++)	{
		...
		const float *d[3], *or_[3];
		for (int i = 0; i < 3; i++)
			d[i] = der1f[i].ptr<float>(r), or_[i] = ornt1f[i].ptr<float>(r);
		for (int c = 0; c < sz.width; c++){
			float maxD = d[0][c];
			int idx = 0;
			for (int i = 1; i < 3; i++)
...
/************************************************************************/
/* ����isForwardΪTRUE������_pOrnt����Ѱ��crtEdge, ������;����_pNext */
/* ��Ӧֵ��Ϊ��;�ķ���ֵ, ͬʱ��_pLineInd��ֵ��Ϊ��ǰ�ߵı���,�Ҳ���  */
/* ��һ����ʱ��������һ������������ΪcrtEdge��End����.                  */
/* ����isForwardΪFALSE������_pOrnt������Ѱ��crtEdge, ������;����     */
/* _pNext��Ӧֵ��Ϊ��;�ķ�������ֵ, ͬʱ��_pLineInd��ֵ��Ϊ��ǰ�ߵ�  */
...
bool CmCurveEx::jumpNext(Point &pnt, float &ornt, CEdge &crtEdge, int orntInd, bool isBackward)
{
	...
	if (CHK_IND(pnt2) && _pLabel1i.at<int>(pnt2) <= IND_NMS) {
		if (angle(ornt, _pOrnt1f.at<float>(pnt2)) > _maxAngDif) //�����õ㷽���뵱ǰ�߷��������Ƚϴ��򲻼���
			return false;
