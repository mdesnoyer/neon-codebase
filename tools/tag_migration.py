...
def main():
    ...
    videos = yield neondata.VideoMetadata.get_all(async=True)
    for v in videos:
        _log.info('Start %s' % v.get_id())
        # Only for test db.
        if v.tag_id:
            t = yield neondata.Tags.get(v.tag_id, async=True)
            r = yield neondata.NeonApiRequest.get(v.job_id, async=True)
            t.name = r.video_title
            yield t.save(async=True)
        else:
            t = neondata.Tag(
                None,
                account_id=v.get_account_id(),
            ...
            tag_id = t.get_id()
            v.tag_id = tag_id
            yield v.save(async=True)

        tids = v.get_all_thumbnail_ids()
        yield TagThumbnail.save(tag_id = tag.tag_id, tids, async=True)
