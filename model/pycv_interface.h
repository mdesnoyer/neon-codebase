/**
Utilities to convert from python to/from opencv.

Grabbed from cv2.cpp in the opencv code.

Note that this is a header library because the NumPy functions are
defined as macros and need import_array() to be called in the
initialization code for the module.

*/

#ifndef __PYCV_INTERFACE_H__
...
int pyopencv_to(const PyObject* o,
  ...
  if (needcopy) {
    if (info.outputarg) {
      failmsg("Layout of the output array %s is incompatible with cv::cv::Mat (step[ndims-1] != elemsize or step[1] != elemsize*nchannels)", info.name);
      return false;
    }
    if( needcast )
  ...
  if( m.data ) {
    ...
    if (!needcopy) {
      ...
      // (since cv::Mat destructor will decrement the reference counter)
    }
  };
  m.allocator = &g_numpyAllocator;
