...
public class TestNeonAvroEventSerializer {
  ...
  public void testOneSchema() throws IOException {
	...
	JSONObject schemaJson = new JSONObject(
			TrackerEvent.getClassSchema().toString());
	...
	GenericRecord record  =
        buildDefaultGenericEvent(schema)
	    .set("eventType", "IMAGE_VISIBLE")
  ...
  public void testSchemaEvolution() throws IOException {
	...
	for(int i = 0; i < 10; i++){
		if(i < 3 && i > 8){
			JSONObject schemaJson = new JSONObject(
					TrackerEvent.getClassSchema().toString());
			schemaJson.getJSONArray("fields").put(new JSONObject(
					"{\"name\": \"dummyField\", \"type\" : [ \"null\" , \"int\" ], \"default\" : \"null\"}"));
			
			schema = new Schema.Parser().parse(schemaJson.toString());
		    
			record  =
	   	        buildDefaultGenericEvent(schema).set("dummyField", 78)
		        .set("eventType", "IMAGE_VISIBLE")
		        .set("eventData", visEvent).build();
		} else {
			JSONObject schemaJson = new JSONObject(
  ...
  public void testBadSchema() throws IOException {
	...
	String badSchema =
	        "{\"type\":\"record\", \"name\":\"com.neon.Tracker.TrackerEvent\", \"fields\":[{\"name\":\"pageUrl\", \"type\":\"string\"}]}";
	    
		       
	JSONObject schemaJson = new JSONObject(badSchema);
	
	schema = new Schema.Parser().parse(schemaJson.toString());
	
	GenericRecord record  = new GenericRecordBuilder(schema)
	        .set("pageUrl", "Yes").build();
  ...
  public void validateAvroFile(File file, GenericRecord origRecord[]) throws IOException {
    ...
    while (fileReader.hasNext()) {
      ...
      Assert.assertTrue(fileReader.next(record).toString()
    		  .startsWith(origRecord[numEvents].toString()));
      numEvents++;
    }
    fileReader.close();
