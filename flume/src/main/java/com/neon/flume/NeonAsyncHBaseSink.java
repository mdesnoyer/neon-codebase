...
*/
public class NeonAsyncHBaseSink extends AbstractSink implements Configurable {

  private String tableName;
  private byte[] columnFamily;
  private long batchSize;
  private static final Logger logger =
          LoggerFactory.getLogger(NeonAsyncHBaseSink.class);
  ...
  public NeonAsyncHBaseSink(){
    this(null);
  }

  public NeonAsyncHBaseSink(Configuration conf) {
    this(conf, false, false);
  }

  @VisibleForTesting
  NeonAsyncHBaseSink(Configuration conf, boolean isTimeoutTest,
  ...
  private class FailureCallback<R,T> implements Callback<R,T> {
    ...
    private void doCall(T error) throws Exception {
      callbacksReceived.incrementAndGet();
      this.txnFail.set(true);
      logger.error("Error from hbase", error);
      lock.lock();
      try {
        condition.signal();
      } finally {
