...
class TestLogger(unittest.TestCase):
    ...
    def stress_from_db():
        test_account_id = "neontest" ; test_integration_id = "i12345"
        api_key = '0be8df123665c9a5f4c5e69261b65b75'
        headers = {'X-Neon-API-Key' : api_key }
        #url = 'http://localhost:8083/api/v1/accounts/test_new_account1382403906.09/brightcove_integrations/i12345/videos'
        #get all videos from the account
        #req = urllib2.Request(url,None,headers)
        #response = urllib2.urlopen(req)
        #items = json.loads(response.read())

        playlist_call = 'http://api.brightcove.com/services/library?command=find_playlist_by_id'
        #response = urllib2.urlopen(req)
        #items = json.loads(response.read())
        load_urls = #fetch from playlist  
        click_url = #pick one random video thumbnail 
