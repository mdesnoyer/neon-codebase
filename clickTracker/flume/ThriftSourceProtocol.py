...
class Iface(object):
  def append(self, event, callback):
    """
    Parameters:
     - event
    """
  ...
  def appendBatch(self, events, callback):
    """
    Parameters:
     - events
    """
...
class Client(Iface):
  ...
  def send_append(self, event):
    ...
    oprot.writeMessageBegin('append', TMessageType.CALL, self._seqid)
    args = append_args()
    args.event = event
    args.write(oprot)
  ...
  def recv_append(self, iprot, mtype, rseqid):
    ...
    if result.success is not None:
      ...
      return
    callback(TApplicationException(TApplicationException.MISSING_RESULT, "append failed: unknown result"))
    return
  ...
  def send_appendBatch(self, events):
    oprot = self._oprot_factory.getProtocol(self._transport)
    oprot.writeMessageBegin('appendBatch', TMessageType.CALL, self._seqid)
  ...
  def recv_appendBatch(self, iprot, mtype, rseqid):
    callback = self._reqs.pop(rseqid)
    if mtype == TMessageType.EXCEPTION:
      x = TApplicationException()
      x.read(iprot)
    ...
    if result.success is not None:
      callback(result.success)
      return
    callback(TApplicationException(TApplicationException.MISSING_RESULT, "appendBatch failed: unknown result"))
    return
...
class Processor(Iface, TProcessor):
  ...
  @gen.engine
  def process_append(self, seqid, iprot, oprot, callback):
    args = append_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = append_result()
    result.success = yield gen.Task(self._handler.append, args.event)
  ...
  @gen.engine
  def process_appendBatch(self, seqid, iprot, oprot, callback):
    args = appendBatch_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = appendBatch_result()
    result.success = yield gen.Task(self._handler.appendBatch, args.events)
    oprot.writeMessageBegin("appendBatch", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
    callback()
...
class appendBatch_args:
  ...
  thrift_spec = (
    None, # 0
    (1, TType.LIST, 'events', (TType.STRUCT,(ThriftFlumeEvent, ThriftFlumeEvent.thrift_spec)), None, ), # 1
  )
