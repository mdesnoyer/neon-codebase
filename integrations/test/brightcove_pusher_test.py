...
class TestCMSAPIPush(test_utils.neontest.AsyncHTTPTestCase):
    ...
    def setUp(self):
        ...
        thumbs = [neondata.ThumbnailMetadata(key, ttype=type,rank=0) for
    ...
    def test_bc_server_error(self):
        ...
        with self.assertLogExists(logging.ERROR, 'Error with Brightcove API'):
            with self.assertRaises(tornado.httpclient.HTTPError) as e:
                response = yield self.submit_callback({
                'serving_url': 'http://neon-images.com/neonvid_vid1.jpg',
                'video_id' : 'vid1',
    ...
    def test_unauthorized_client_tokens(self):
        ...
        with self.assertLogExists(logging.ERROR, 'No valid Brightcove'):
            with self.assertRaises(tornado.httpclient.HTTPError) as e:
                response = yield self.submit_callback({
                'serving_url': 'http://neon-images.com/neonvid_vid1.jpg',
                'video_id' : 'vid1',
